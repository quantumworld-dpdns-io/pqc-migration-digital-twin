...
- observability/health hardening in core services (`src/go/gateway`, `src/go/discovery`, `src/python/service.py`, `src/rust/risk-service`) with request IDs, structured logs, and `/live` + `/ready` probes.
