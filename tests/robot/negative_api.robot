...
Negative Malformed JSON On Governance Exceptions Endpoint Returns 400
    ...
    ${response}=    POST    ${GATEWAY_BASE_URL}/api/v1/governance/exceptions    data={"asset_id":"A1",    headers=${headers}    expected_status=400
    Should Be Equal As Integers    ${response.status_code}    400

Negative Invalid Limit On Audit Events Returns 400
    ${response}=    GET    url=${GATEWAY_BASE_URL}/api/v1/audit/events?limit=-1    expected_status=400
    Should Be Equal As Integers    ${response.status_code}    400
